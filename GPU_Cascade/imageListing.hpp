	...
	public:
		...
		int imageListing()
		{
			...
			for (int i = 0; i < group_count; i++) {
				...
				for (int j = 0; j < block_count_per_group; j++) {
					char temp_j[2] = { ' ','\0' };
					temp_j[0] = j + 48;
					const std::string str_j = temp_j;
					const std::string sImageInputDir = sImageInputDirFather + "DJI_" + str_i + "/" + "DJI_" + str_i + "_" + str_j + "/";
					const std::string sImageListingOutputDir = sImageListingOutputDirFather + "DJI_" + str_i + "/" + "DJI_" + str_i + "_" + str_j + "_build/";
