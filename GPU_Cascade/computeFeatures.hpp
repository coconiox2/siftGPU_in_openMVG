#pragma once
#ifndef _COMPUTE_FEATURES_H
#define _COMPUTE_FEATURES_H
...
namespace computeFeatures {
	...
	const std::string sComputeFeaturesOutDir = stlplus::folder_up(imageInputDir, 4) +
		"/imageData/360_school/build/";

	class ComputeFeatures {
	public:
		///check if sSfM_Data_FilenameDir,sSfM_Data_Filename,sComputeFeaturesOutDir is valid
		...
		bool checkFileAndPathValidity()
		{
			...
			if (!stlplus::is_folder(sComputeFeaturesOutDir))
			{
				std::cerr << "sComputeFeaturesOutDir:" << sComputeFeaturesOutDir << "is not a stlplus::folder!\n" << std::endl;
				return false;
			}
		...
		int computeFeatures()
		{
			...
			//		cmd.add(make_option('o', sComputeFeaturesOutDir, "outdir"));
			//		// Optional
			...
			std::cout << " You called : " << std::endl
				...
				<< "--outdir " << sComputeFeaturesOutDir << std::endl
			...
			if (sComputeFeaturesOutDir.empty()) {
				std::cerr << "\nIt is an invalid output directory" << std::endl;
				return EXIT_FAILURE;
			}

			// Create output dir
			if (!stlplus::folder_exists(sComputeFeaturesOutDir))
			{
				if (!stlplus::folder_create(sComputeFeaturesOutDir))
				{
			...
			const std::string sImage_describer = stlplus::create_filespec(sComputeFeaturesOutDir, "image_describer", "json");
...
#pragma omp parallel for schedule(dynamic) if (iNumThreads > 0) private(imageGray)
				...
				for (int i = 0; i < static_cast<int>(sfm_data.views.size()); ++i)
				{
					...
					const std::string
						...
						sFeat = stlplus::create_filespec(sComputeFeaturesOutDir, stlplus::basename_part(sView_filename), "feat"),
						sDesc = stlplus::create_filespec(sComputeFeaturesOutDir, stlplus::basename_part(sView_filename), "desc");
